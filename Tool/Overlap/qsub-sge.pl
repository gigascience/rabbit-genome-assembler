...
my ($Global, $Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Job_prefix,$Verbose, $Help,$Project);
GetOptions(
	"global"=>\$Global,
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
...
while (@Shell) {
	...
	$job_cmd .= "-P $Project "  if(defined $Project); ##set queue
...
if (!$Verbose) {
	`rm -r $work_shell_file_globle $Work_dir`;
}
